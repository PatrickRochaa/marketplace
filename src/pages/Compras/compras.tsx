...
// Tipagem para os dados de compras
type Sale = {
  nome_produto: string;
  quantidade: number;
  preco: number;
...
export function Compras() {
  ...
  const [salesData, setSalesData] = useState<Sale[]>([]); // Para armazenar as compras
  const [selectedPeriod, setSelectedPeriod] = useState<string>("all"); // Padrão: últimos 10 dias
  const [loading, setLoading] = useState(false); // Estado para indicar carregamento
  const [totalComprado, setTotalComprado] = useState<number>(0); // Estado para o total gasto
  ...
  async function fetchSales(period: string) {
    ...
    try {
      ...
      const calculateTotal = (sales: Sale[]) => {
        return sales.reduce(
          (total, sale) => total + sale.quantidade * sale.preco,
          0
        );
      };

      // Atualiza o estado com as transações e o total gasto
      const allSales = filteredData.flatMap((transaction: Transaction) => {
        const transactionItems = transaction.itens.map((item: Item) => {
          ...
          <section className={styles.salesTableContainer}>
            ...
            {loading ? (
              ...
              <>
                ...
                {/* Total das Compras*/}
                <div className={styles.totalComprado}>
                  <span>
                    <strong>Total:</strong>
                  </span>
                  <span>
