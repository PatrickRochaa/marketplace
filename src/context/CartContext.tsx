...
import { ProductProps } from "../types/products"; // interface products
...
export function CartProvider({ children }: CartProviderProps) {
  ...
  const syncCart = async () => {
    ...
    if (storedCart.length > 0) {
      try {
        ...
        const { data: supabaseCart, error: supabaseError } = await supabase
          .from("carrinho")
        ...
        if (supabaseCart.length === 0) {
          ...
          for (const item of storedCart) {
            const existingProduct = supabaseCart.find(
              (prod) => prod.produto_id === item.id
            );
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      if (!user) return; // Se não houver usuário logado, não tenta carregar o carrinho
      ...
      try {
        ...
        const productPromises = data.map(async (item: CartItemData) => {
  ...
  const addToCart = async (product: ProductProps, quantity: number = 1) => {
    ...
    if (productData && productData.user_id === user.id) {
      toast.error(
        "Você não pode adicionar um produto que você mesmo cadastrou ao carrinho."
      );
    ...
    try {
      ...
      if (existingProducts.length > 0) {
        // Soma as quantidades
        const totalQuantity =
          existingProducts.reduce((sum, item) => sum + item.quantidade, 0) +
          quantity;
  ...
  const updateQuantity = async (productId: number, quantity: number) => {
    ...
    if (quantity < 1) {
      ...
      try {
        ...
        if (quantity < 1) {
          await supabase.from("carrinho").delete().eq("id", cartItem.id);
        ...
        setCartItems((prevItems) =>
          prevItems.map((item) =>
            item.id === productId
              ? { ...item, quantity, preco_total: newPrecoTotal }
              : item
          )
        );
